...
// declare constants
const intToOutcome = ["Aleksa Wins!", "Draw", "Milica Wins!"];
...
class Player extends React.Component {
  ...
  async getResult() {
    const result = await new Promise((resolveResultP) => {
      this.setState({ view: "GetResult", playable: true, resolveResultP });
    });
    this.setState({ view: "WaitingForResults", result });
    return result;
  }
  seeOutcome(i) {
  ...
  playResult(result) {
    this.state.resolveResultP(result);
  }
}
